...
- (void)viewDidLoad {
    ...
    /* //reshape，step，矩阵打印
     cv::Mat testMat = cv::Mat::zeros ( 5, 4, CV_8UC3 ); //5行4列，3通道，5行4列3通道
     std::cout << "size of testMat: " << testMat.rows << " x " << testMat.cols << std::endl;
     std::cout<<"testMat = "<<testMat<<std::endl;
     std::cout<<testMat.step[0]<<std::endl; //testMat.dims 维度，step[0]代表行的大小，step[1]代表每个元素的大小
     std::cout<<testMat.step[1]<<std::endl;
     //reshape方法
     cv::Mat result = testMat.reshape ( 0, 2 ); //0表示通道数不变，2表示行，那么列就变为10，3通道
    ...
    /*
     //3x3矩阵，reshape和resize
     cv::Mat M = (cv::Mat_<uchar>(3,3) << 1,2,3,4,5,6,7,8,9);
     std::cout<<"原矩阵：\n"<<M<<std::endl;
     std::cout<<"mat::reshape:\n"<<M.reshape(0,1)<<std::endl; //reshape矩阵个数不变
     
     M.resize(1); //变为1行
     std::cout<<"mat::resize:\n"<<M<<std::endl; //resize个数会变
     */
    ...
    /*
     ...
     cv::Mat image;
     image = cv::imread(cpath,CV_LOAD_IMAGE_COLOR);
     NSLog(@"%d--%d--%d",image.rows,image.cols,image.channels()); //row为高，cols为宽
     */
...
{
    int m = image.rows; //行数
    int n = image.cols*image.channels(); //列数
...
void colorReduce2(cv::Mat image, int div = 16)
{
    ...
    int m = image.rows; //行数
    int n = image.cols*image.channels(); //b列数
